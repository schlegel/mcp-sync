...
function buildHelp(): string {
  const lines = [
    ...
    '',
    `  ${dim('Usage:')}  ${white('owl07')} ${green('<command>')} ${dim('[options]')}`,
    '',
    section('Setup'),
    cmd('init', 'Create .owl07.json in current project'),
    ...
    cmd('watch', 'Watch .owl07.json and auto-sync on changes'),
    '',
    section('Examples'),
    `  ${dim('$')} ${white('owl07 init')}`,
    `  ${dim('$')} ${white('owl07 use fullstack')}`,
    `  ${dim('$')} ${white('owl07 add-json github \'{"command":"npx","args":["-y","@modelcontextprotocol/server-github"]}\'')}`,
    `  ${dim('$')} ${white('owl07 sync')}`,
    `  ${dim('$')} ${white('owl07 doctor')}`,
    '',
    `  ${dim('Docs:')}  ${glow('https://github.com/aditya-ai-architect/owl07')}`,
    `  ${dim('npm:')}   ${glow('https://www.npmjs.com/package/owl07')}`,
    '',
  ];
